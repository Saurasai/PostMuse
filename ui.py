                ...
                for i, draft in enumerate(drafts, 1):
                    ...
                    with st.expander(f"📅 Schedule Draft {i}"):
                        try:
                            ...
                            with col2:
                                schedule_time_input = st.time_input("⏰ Time", key=f"{platform}_{i}_clock")
                            ...
                            if st.button(f"Schedule", key=f"{platform}_{i}_btn"):
                                if st.session_state.logged_in_user:
                                    ...
                                    st.success(f"Draft scheduled for {schedule_time} UTC")
                                    logger.info(f"Draft scheduled for {st.session_state.logged_in_user} on {platform} at {schedule_time}")
                                    st.rerun()
                                else:
    ...
    if st.session_state.logged_in_user:
        with tabs[3]:
            ...
            if not posts:
                ...
                for post in posts:
                    post_id, platform, content, sched_time = post
                    col1, col2 = st.columns([3, 1])
                    with col1:
                        st.markdown(f"**Platform:** {platform.capitalize()}")
                        st.markdown(f"**Scheduled Time (UTC):** {sched_time}")
